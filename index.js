...
function checkBoxes(e) {
	...
	if (!curr.classList.contains("flagged")) {
		...
		/**
		 *  click on number. 
		 * reveal this box only (prevent next else to shoot checkSides)
		 */
		else if (curr.innerHTML > 0 && curr.innerHTML < 9) {}

		/**
		 * click on empty. 
		 * reveal this box + reveal linked area until number
		 */
		else {
...
function checkRowCell(curr, roww, coll) {
	if (mineBody.rows[roww].cells[coll] &&
		...
		/**
		 * function callback with new row-col if box is empty
		 */
